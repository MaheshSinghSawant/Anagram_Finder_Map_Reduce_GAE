...
class StoreOutput(base_handler.PipelineBase):
  ...
  def run(self, mr_type, encoded_key, output):
    ...
    base_path = output[0].rpartition('/')[0]
    with gcs.open(base_path + '/result_file', 'w', content_type='text/plain') as f:
        for counter in range(0, len(output)):
            blobstore_filename = "/gs" + output[counter]
            blobstore_gs_key = blobstore.create_gs_key(blobstore_filename)
            blob_reader = blobstore.BlobReader(blobstore_gs_key)
            value = blob_reader.read()
            f.write(value)
    blobstore_filename = "/gs" + base_path + '/result_file'
    blobstore_gs_key = blobstore.create_gs_key(blobstore_filename)
